...
| 2 Real tracing             | Implemented, Docker validation required | GDB/DWARF records and linked allocation events; never source-inferred           |
...
| 7 Hardening                | Implemented baseline                    | container policy and adversarial harness; microVM not included                  |
...
- Docker golden/security suites: require Docker; not available in every authoring
  environment and must pass in CI before a release is tagged.
